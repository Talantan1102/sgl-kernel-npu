# Adapted from https://github.com/vllm-project/vllm-ascend/pull/6091
#   (vllm_ascend/ops/triton/fla/chunk_delta_hupdate.py)
# Original: https://github.com/fla-org/flash-linear-attention
# -*- coding: utf-8 -*-
...
def chunk_gated_delta_rule_fwd_hupdate(
    ...
    h_update[:, update_indices, :, :, :] = torch.eye(
        K, dtype=h_update.dtype, device=h_update.device
    )
    ...
    h_update[:, : num_decodes * 2, :, :, :] = torch.zeros(
        (K, K), dtype=h_update.dtype, device=h_update.device
    )
    return h_update
